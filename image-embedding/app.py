...
AUDIO_MAX_RESULTS = 4  # ค่าเริ่มต้น (ผู้ใช้ปรับได้จาก UI ในช่วง AUDIO_MAX_RESULTS_RANGE)
AUDIO_MAX_RESULTS_RANGE = (1, 10)
AUDIO_SCORE_THRESHOLD_DEFAULT = 0.0
AUDIO_SCORE_THRESHOLD_RANGE = (0.0, 1.0)
...
def classify_audio(
    path: Path,
    max_results: int = AUDIO_MAX_RESULTS,
    score_threshold: float = AUDIO_SCORE_THRESHOLD_DEFAULT,
) -> dict:
    """
    ...
    เพื่อให้รองรับไฟล์เสียงความยาวเท่าไหร่ก็ได้ และรองรับปรับ max_results /
    score_threshold จากหน้าเว็บ (เหมือน MODEL SELECTION / SETTINGS ใน mediapipe-samples-web)

    คืนค่า: {"segments": [...], "inference_time_ms": float}
    """
    base_options = mp_python.BaseOptions(model_asset_path=str(AUDIO_MODEL_PATH))
    options_kwargs = {"base_options": base_options, "max_results": max_results}
    # score_threshold=0 ใน MediaPipe แปลว่า "ไม่กรองอะไรเลย" อยู่แล้ว แต่เราส่งเฉพาะตอน > 0
    # เพื่อความชัดเจน (และเลี่ยงพฤติกรรมที่อาจต่างกันในแต่ละเวอร์ชันของไลบรารี)
    if score_threshold > 0:
        options_kwargs["score_threshold"] = score_threshold
    options = audio.AudioClassifierOptions(**options_kwargs)
    ...
    return {"segments": segments, "inference_time_ms": round(inference_time_ms, 1)}
...
def _parse_max_results(raw_value: str) -> int:
    lo, hi = AUDIO_MAX_RESULTS_RANGE
    try:
        value = int(float(raw_value))
    except (TypeError, ValueError):
        return AUDIO_MAX_RESULTS
    return max(lo, min(hi, value))


def _parse_score_threshold(raw_value: str) -> float:
    lo, hi = AUDIO_SCORE_THRESHOLD_RANGE
    try:
        value = float(raw_value)
    except (TypeError, ValueError):
        return AUDIO_SCORE_THRESHOLD_DEFAULT
    return max(lo, min(hi, value))
...
def api_classify_audio():
    ...
    try:
        ...
        max_results = _parse_max_results(request.form.get("max_results"))
        score_threshold = _parse_score_threshold(request.form.get("score_threshold"))

        result = classify_audio(converted_path, max_results, score_threshold)
        ...
        return jsonify(
            {
                ...
                "settings": {"max_results": max_results, "score_threshold": score_threshold},
            }
        )
